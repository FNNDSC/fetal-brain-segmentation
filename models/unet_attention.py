...
def getAttentionUnet():
    ...
    # Decoding (upwards)
    att1 = attention_block(256, UpSampling2D(size = (2,2))(drop5), drop4)
    merge6 = concatenate([att1, drop4], axis = 3)
    conv6 = Conv2D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge6)
    conv6 = Conv2D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv6)

    att2 = attention_block(128, UpSampling2D(size = (2,2))(conv6), conv3)
    merge7 = concatenate([att2, conv3], axis = 3)
    conv7 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge7)
    conv7 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv7)

    att3 = attention_block(64, UpSampling2D(size = (2,2))(conv7), conv2)
    merge8 = concatenate([att3, conv2], axis = 3)
    conv8 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge8)
    conv8 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv8)

    att4 = attention_block(32, UpSampling2D(size = (2,2))(conv8), conv1)
