...
def squeeze_excite_block(input, ratio=16):
    ...
    se = layer.GlobalAveragePooling2D()(init)
    se = layer.Reshape(se_shape)(se)
    se = layer.Dense(filters // ratio, activation='relu', kernel_initializer='he_normal', use_bias=False)(se)
    se = layer.Dense(filters, activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(se)

    x = layer.multiply([init, se])
    return x
